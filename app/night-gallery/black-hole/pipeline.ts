import type { Effect, Frame, Gpu, Surface, Target, Timer } from "vgpu";
...
// Resize existing owners: vGPU releases the old attachments and retains
// resource identity. Rebind the MRT color views after resizing.
export function resizeTargets(targets: Targets, size: readonly [number, number]): void {
  const full = normalizeSize(size);
  targets.gbuffer.resize(full); targets.aa.resize(full); targets.scene.resize(full);
  targets.bloom0.resize(scaleSize(full, 2)); targets.bloomPing0.resize(scaleSize(full, 2));
  targets.bloom1.resize(scaleSize(full, 4)); targets.bloomPing1.resize(scaleSize(full, 4));
  targets.bloom2.resize(scaleSize(full, 8)); targets.bloomPing2.resize(scaleSize(full, 8));
}
...
export function renderChain(
  ...
  if (bake) {
    frame.pass({ target: targets.gbuffer, clear: CLEAR, timer: timer?.span("gbuffer") }, (pass) =>
      pass.draw(effects.bake)
    );
    frame.pass({ target: targets.aa, clear: CLEAR, timer: timer?.span("aa") }, (pass) =>
      pass.draw(effects.refine)
    );
  }
  frame.pass({ target: targets.scene, clear: CLEAR, timer: timer?.span("scene") }, (pass) =>
    pass.draw(effects.shade)
  );
  frame.pass({ target: targets.bloom0, clear: CLEAR, timer: timer?.span("bloom0") }, (pass) =>
    pass.draw(effects.bloomExtract)
  );
  frame.pass({ target: targets.bloomPing0, clear: CLEAR, timer: timer?.span("bloomPing0") }, (pass) =>
    pass.draw(effects.bloomBlurH0)
  );
  frame.pass({ target: targets.bloom0, clear: CLEAR, timer: timer?.span("bloom0Vertical") }, (pass) =>
    pass.draw(effects.bloomBlurV0)
  );
  frame.pass({ target: targets.bloom1, clear: CLEAR, timer: timer?.span("bloom1") }, (pass) =>
    pass.draw(effects.bloomDown1)
  );
  frame.pass({ target: targets.bloomPing1, clear: CLEAR, timer: timer?.span("bloomPing1") }, (pass) =>
    pass.draw(effects.bloomBlurH1)
  );
  frame.pass({ target: targets.bloom1, clear: CLEAR, timer: timer?.span("bloom1Vertical") }, (pass) =>
    pass.draw(effects.bloomBlurV1)
  );
  frame.pass({ target: targets.bloom2, clear: CLEAR, timer: timer?.span("bloom2") }, (pass) =>
    pass.draw(effects.bloomDown2)
  );
  frame.pass({ target: targets.bloomPing2, clear: CLEAR, timer: timer?.span("bloomPing2") }, (pass) =>
    pass.draw(effects.bloomBlurH2)
  );
  frame.pass({ target: targets.bloom2, clear: CLEAR, timer: timer?.span("bloom2Vertical") }, (pass) =>
    pass.draw(effects.bloomBlurV2)
  );
  frame.pass({ target: output, clear: CLEAR, timer: timer?.span("composite") }, (pass) =>
